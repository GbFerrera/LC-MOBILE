...
export default function AgendaScreen({ navigation }: any) {
  ...
  const { bottomSheetRef: detailsBottomSheetRef, openBottomSheet: openDetailsBottomSheet, closeBottomSheet: closeDetailsBottomSheet } = useBottomSheet();
  ...
  const [filteredServices, setFilteredServices] = useState<{id: number, name: string, price: number}[]>([]);
  ...
  const fetchServices = useCallback(async () => {
    try {
      ...
      if (token && companyId) {
        // Buscar serviços disponíveis para o profissional logado
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Bottom Sheet para Criar Agendamento */}
      <BottomSheetModal
        ref={bottomSheetRef}
        snapPoints={['70%', '90%']}
        onClose={() => {
            ...
            <Button
              ...
              onPress={async () => {
                try {
                  ...
                  // Enviar requisição para criar agendamento
                  ...
                  // Fechar o bottom sheet e atualizar a lista de agendamentos
                  closeBottomSheet();
                  fetchAppointments();
                } catch (error) {
          ...
          <View style={{ gap: 16 }}>
            ...
            <View style={styles.buttonContainer}>
              <Button
                mode="outlined"
                style={styles.cancelButton}
                onPress={closeDetailsBottomSheet}
              >
                Fechar
              </Button>
